...
var ajaxtooltip={
	...
	loadingHTML: '<div style="font-style:italic"><img src="/images/ajaxload.gif" /> Get comments...</div>',
	...
	showtip:function($tooltip, e){
		if ( !$tooltip.get(0).showFlag ) return;
...
function ajaxtooltip_initialize() {
	...
	$('.ajaxtooltip').each(function(index){ //find all links with "title=ajax:" declaration
		...
		$target.hover(
			...
			function(e){ //onMouseout element
				var $tooltip=tooltips[parseInt(this.titleposition)];
				$tooltip.get(0).showFlag = false;
				$tooltip.get(0).loadsuccess = false;
	...
	$('.clickTip').each(function(index){ //find all links with "title=ajax:" declaration
		this.titleposition=index+' pos' //remember this tooltip DIV's position relative to its peers
		var $target=$(this)
		tooltips.push($('<div class="ajaxtooltip-box">'+$target.attr("title")+'</div>').appendTo('body'))
