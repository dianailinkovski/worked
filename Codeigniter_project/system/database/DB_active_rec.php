...
class CI_DB_active_record extends CI_DB_driver {
	...
	// Custom modification for sql query cache, justSticky
	var $ar_SQL_NO_CACHE		= false;
	var $ar_SQL_CACHE			= false;
	...
	function select($select = '*', $escape = NULL)
	{
		...
		//reset sql cache vars to Off, for each new query, just to be safe
		$this->ar_SQL_CACHE = false;
		$this->ar_SQL_NO_CACHE = false;
	 ...
	 * Mysql Cache Off
	 *
	 * Turns Off MYSQL_CACHE
	 *
	 * @access	public
	 * @param	NULL
	 * @return	object
	 */
	function mysql_no_cache()
	{
		$this->ar_SQL_CACHE = false;
		$this->ar_SQL_NO_CACHE = true;
		return $this;
	}
	...
	function set($key, $value = '', $escape = TRUE)
	{
		...
		foreach ($key as $k => $v)
		{
			if ($escape === FALSE)
			{
				$this->ar_set[$this->_protect_identifiers($k)] = trim($v);
			}
			else
			{
	 ...
	 * @return	object
	 */
	function get($table = '', $limit = null, $offset = null, $dump = null)
	{
		if ($table != '')
		{
			$this->_track_aliases($table);
	 ...
	 * @param	bool    mysql INSERT IGNORE
	 * @return	object
	 */
	function insert($table = '', $set = NULL, $insert_ignore_flag=false, $insert_low_priority_flag=false)
	{	
		if ( ! is_null($set))
		{
			$this->set($set);
		...
		$sql = $this->_insert($this->_protect_identifiers($table, TRUE, NULL, FALSE), array_keys($this->ar_set), array_values($this->ar_set), $insert_ignore_flag, $insert_low_priority_flag);
	...
	function _compile_select($select_override = FALSE)
	{
		...
		{
			$sql = 'SELECT ';
			$sql .= ($this->ar_SQL_CACHE) 		? 'SQL_CACHE ': '';
			$sql .= ($this->ar_SQL_NO_CACHE) 	? 'SQL_NO_CACHE ': '';
