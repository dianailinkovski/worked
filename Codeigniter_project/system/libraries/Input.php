	...
	*/
	function _html_entity_decode($str, $charset='UTF-8')
	{
		...
		*/
		...
		// change by Christophe
		// https://github.com/bcit-ci/CodeIgniter/issues/2681
		...
		if (function_exists('html_entity_decode') && (strtolower($charset) != 'utf-8' OR version_compare(phpversion(), '5.0.0', '>=')))
		{
			...
			{
				$str = preg_replace_callback('~&#x(0*[0-9a-f]{2,5})~i', function(){return chr(hexdec('\\1'));}, $str);
				$str = preg_replace_callback('~&#([0-9]{2,4})~', function(){return chr('\\1');}, $str);
			}
			else
			{
				$str = preg_replace('~&#x(0*[0-9a-f]{2,5})~ei', 'chr(hexdec("\\1"))', $str);
		...
		if (version_compare(PHP_VERSION,5.5,'>='))
		{
			$str = preg_replace_callback('~&#x(0*[0-9a-f]{2,5});{0,1}~i', function(){return chr(hexdec('\\1'));}, $str);
			$str = preg_replace_callback('~&#([0-9]{2,4});{0,1}~', function(){return chr('\\1');}, $str);
		}
		else
		{
			$str = preg_replace('~&#x(0*[0-9a-f]{2,5});{0,1}~ei', 'chr(hexdec("\\1"))', $str);
