...
/**
 * CActiveRecord version of uploading behavior.
 * 
 ...
 *  
 * All you need to do to make it work is attach the behavior to a CActiveRecord (and set up the validation rules), 
 ...
 * Here is an example :
 * 
 * Attach the behavior to a CActiveRecord :
 * 
 * <pre>
 ...
 * @package Behavior
 */
class ActiveRecordUploadingBehavior extends CActiveRecordBehavior
{
	/**
	...
	/**
	 * Deletes the files before deleting the model.
	 */
		...
		if ($owner->$attribute != '')
		{
			if (isset($this->formats))
			{
				foreach ($this->formats as $format => $dimensions)
					if (file_exists(($file = $this->dir.'/'.$this->fileSuffix($owner->$attribute, $format))))
						unlink($file);
						
				if (file_exists(($file = $this->dir.'/'.$this->fileSuffix($owner->$attribute, 'a'))))
					unlink($file);
			}
			else {
				if (file_exists(($file = $this->dir.'/'.$owner->$attribute)))
					unlink($file);
			}
		}
	}
