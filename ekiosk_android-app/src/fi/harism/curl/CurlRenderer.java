...
public class CurlRenderer implements GLSurfaceView.Renderer {
	...
	public CurlRenderer(CurlRenderer.Observer observer, float ratio) {
		issueRatio = ratio;
		mObserver = observer;
		mCurlMeshes = new Vector<CurlMesh>();
		mPageRectLeft = new RectF();
	...
	public synchronized void onDrawFrame(GL10 gl) {
		...
		gl.glTranslatef(currentX, currentY, -currentScale);
	...
	public void zoom(float scaleAbsolute, float focusX, float focusY) {
		...
		currentScale = scaleAbsolute;
		
		float ratio = (float) mViewportWidth / mViewportHeight;
		
		focusX = focusX  * ratio / 2000 * currentScale;
		focusY = focusY  * ratio / 2000 * currentScale;
		
	    //Keep track of deltaX/deltaY here? How?
		if (currentScale == 6.0f) {
			currentX = 0;
			currentY = 0;
		}
		else {
		...
		//Log.v("currentY", String.valueOf(currentY));
		
	}

	@Override
	public void onSurfaceChanged(GL10 gl, int width, int height) {
		Log.w("on surface changed width = ", String.valueOf(width));
		Log.w("on surface changed height = ", String.valueOf(height));
		...
		Log.e("ratio renderer", String.valueOf(issueRatio));
		float ratio = issueRatio;
		mViewRect.top = 1.0f;
